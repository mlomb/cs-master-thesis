...
use crate::method::{eval::EvalRead, Sample};
...
pub fn batch_loader(cmd: BatchLoaderCommand) -> Result<(), Box<dyn Error>> {
    ...
    // mmap the input file
    let mmap = unsafe { MmapOptions::new().map(&file).expect("can't mmap input") };
    let mut reader = Cursor::new(mmap.as_slice());
...
fn build_method(cmd: &BatchLoaderCommand) -> Box<dyn Sample> {
    match cmd.method {
